...
export function generateSchedulePrompt(body: any): string {
  ...
  prompt += `\n📌 요청 사항:\n`;
  prompt += `- 위 날짜별로 적절하게 장소들을 배분해 일정 구성해줘.\n`;
  prompt += `- 장소 간 동선을 고려하고, 하루 내 이동과 시간을 최대한 효율적으로 구성해줘.\n`;
  prompt += `- 장소들은 중복되지 않게 하고, 각 날짜 chunk마다 이미 선택된 장소는 제외해줘.\n`;
  prompt += `- 모든 데이터는 빠짐없이 구성해줘.\n`;
  prompt += `- 결과는 **JSON 형태**로 구성해줘. 날짜를 key로 하는 구조로 아래와 같이:\n`;
  prompt += `  {\n    "YYYY-MM-DD": [\n      {\n        "순서": 1,\n        "start": "09:00",\n        "end": "11:00",\n        "장소": "경주 탈해왕릉",\n        "위도": lat,\n        "경도": lon,\n        "주소": "...",\n        "타입": "관광지",\n        "image": "...",\n        "rating": 평점,\n        "reviewCount": 리뷰수\n      }, ...\n    ], ...\n  }\n`;
  prompt += `- 반드시 JSON 전체를 출력해줘. 설명은 절대 포함하지 말고 JSON만 보내줘.\n`;
...
export function generateSchedulePrompts(
  ...
  return dataTime.map((day) => {
    ...
    prompt += `  {\n    "${date}": [\n      {\n        "순서": 1,\n        "start": "09:00",\n        "end": "11:00",\n        "장소": "경주 탈해왕릉",\n        "위도": lat,\n        "경도": lon,\n        "주소": "...",\n        "타입": "관광지",\n        "image": "...",\n        "rating": 평점,\n        "reviewCount": 리뷰수\n      }\n    ]\n  }\n`;
    prompt += `- 모든 데이터는 반드시 값을 얻어와야하고 장소들이 중복되면 안 돼 \n`;
